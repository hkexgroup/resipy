...
'errCol' : "<h3>User supplied error</h3>\
            <p>By default, if reciprocal quadrupole are present, ResIPy offers the possibility to fit an error model and use it as weight in the inversion. If no reciprocal are present, the <code>a_wgt</code> and <code>b_wgt</code> parameters can be set to fit a default error model</p>\
            <p>The resulting error are placed in the <code>resError</code> and <code>phaseError</code> columns. If the survey imported contains already columns with an error that the user wish to use a weight in the inversion, it can be specified here. It will overwrite any <code>a_wgt</code> or <code>b_wgt</code> values set.",
...
'parallel' : "<p>Run singular inversions in parallel (distributed processing) according to the number of cores "
              "of your computer (or specified below in 'ncores'). Note that you won't be able to see the "
              "progress of the inversion. "
              "WARNING: For 3D surveys the level of RAM required can be significant as multiple 3D datasets are processed simultanouesly"
              " (therefore errors associated with over allocation of memory are more likley).</p>",
'ncores': "<p>Number of logical processors (or threads) that are used during parallel operations (such as "
            "time-lapse inversions), exceeding the number of processors on your computer"
