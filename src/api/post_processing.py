...
def importR3tErr(file_path):
    ...
    if "Observed_Phase" in headers:
        ...
        for i in range(1,len(dump)):
            ...
            error_dict["C"].append(int(line_info[5]))
            error_dict["P+"].append(int(line_info[7]))
            error_dict["P-"].append(int(line_info[9]))
            error_dict["C+"].append(int(line_info[11]))
            error_dict["C-"].append(int(line_info[13]))
        
    return error_dict

def dispNormErr(error_dict, ax=None):
    """
